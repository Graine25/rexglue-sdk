...
class D3D12Presenter final : public Presenter {
 ...
 private:
  ...
  struct PaintContext {
    ...
    // paint usage.
    std::array<std::pair<uint64_t, Microsoft::WRL::ComPtr<ID3D12Resource>>, kGuestOutputMailboxSize>
  ...
  // guest_output_resources_).
  std::unique_ptr<D3D12GPUCompletionTimeline> guest_output_resource_refresher_completion_timeline_;
