...
class VulkanGPUCompletionTimeline : public GPUCompletionTimeline {
 public:
  ...
  VulkanGPUCompletionTimeline& operator=(VulkanGPUCompletionTimeline&&) = delete;

  ~VulkanGPUCompletionTimeline();

  class FenceAcquisition {
   public:
    explicit FenceAcquisition(VulkanGPUCompletionTimeline* const completion_timeline,
                              const VkFence fence)
    ...
    // ReXGlue: xenia spells this operator== (typo), which disables move assignment.
    ...
    ~FenceAcquisition() {
      if (completion_timeline_ && fence_) {
        ...
        if (submission_successful_.value_or(false)) {
          assert_true(completion_timeline_->pending_submission_fences_.empty() ||
    ...
    VkFence GetFenceForSubmitting() {
      ...
      // submit, but failed.
      assert_true(!submission_successful_.has_value() || *submission_successful_);
  ...
  VkResult AcquireFenceAndSubmit(uint32_t queue_family_index, uint32_t queue_index,
