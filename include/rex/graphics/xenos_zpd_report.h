...
struct XenosZPDReport {
  ...
  static uint32_t SaturateSampleCount(uint32_t sample_count) {
    double saturation =
        std::clamp(static_cast<double>(REXCVAR_GET(occlusion_query_saturation)), 0.0, 1.0);
  ...
  static uint32_t QueryBatchFakeSamples(uint32_t& sample_count) {
    ...
    uint32_t range = static_cast<uint32_t>(REXCVAR_GET(occlusion_query_querybatch_range));
