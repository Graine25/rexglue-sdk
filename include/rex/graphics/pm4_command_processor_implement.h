...
#if !defined(NDEBUG)
#define REX_ENABLE_PM4_DISASM 1
#endif

using namespace rex::graphics::xenos;
void COMMAND_PROCESSOR::ExecuteIndirectBuffer(uint32_t ptr, uint32_t count) REX_RESTRICT {
  ...
  if (count != 0) {
    ...
    // Execute commands!
    new (&reader_) RingBuffer(memory_->TranslatePhysical(ptr), count * sizeof(uint32_t));
...
// ReXGlue: static, this header is compiled into every command processor TU.
static std::string GenerateRegnameForPm4Print(uint32_t reg) {
...
REX_NOINLINE
void COMMAND_PROCESSOR::DisassembleCurrentPacket() REX_RESTRICT {
  rex::graphics::PacketInfo packet_info;

  logging::LoggerBatch<LogLevel::Debug> logger{};

  if (PacketDisassembler::DisasmPacket(reader_.buffer() + reader_.read_offset(), &packet_info)) {
    logger("CP - {}, count {}, predicated = {}\n", packet_info.type_info->name, packet_info.count,
           packet_info.predicated);
    ...
    for (auto&& action : packet_info.actions) {
      ...
      switch (action.type) {
        ...
        case PType::kRegRmw: {
          ...
          logger("\t{} = ({} & {}) | {}\n", dest, dest, and_mask_str, or_mask_str);
...
bool COMMAND_PROCESSOR::ExecutePacket() {
#if REX_ENABLE_PM4_DISASM == 1
  if (REXCVAR_GET(disassemble_pm4) && logging::ShouldLog(LogLevel::Debug)) {
  ...
  REX_LIKELY_IF(packet && packet != 0x0BADF00D) {
    REX_LIKELY_IF((packet != 0xCDCDCDCD)) {
    actually_execute_packet:
      // chrispy: reorder checks by probability
      REX_LIKELY_IF(packet_type == 3) {
        return COMMAND_PROCESSOR::ExecutePacketType3(packet);
      }
      else {
        if (packet_type == 0) {  // dont know whether 0 or 1 are the next most frequent
...
*/
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType0(uint32_t packet) REX_RESTRICT {
  // Type-0 packet.
...
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType1(uint32_t packet) REX_RESTRICT {
  // Type-1 packet.
...
bool COMMAND_PROCESSOR::ExecutePacketType2(uint32_t packet) REX_RESTRICT {
  // Type-2 packet.
  // No-op. Do nothing.
  trace_writer_.WritePacketStart(uint32_t(reader_.read_ptr() - 4), 1);
  trace_writer_.WritePacketEnd();
...
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3(uint32_t packet) REX_RESTRICT {
  // Type-3 packet.
  ...
  if (COMMAND_PROCESSOR::GetCurrentRingReadCount() >= count * sizeof(uint32_t)) {
    ...
    switch (opcode) {
      ...
      case PM4_INDIRECT_BUFFER_PFD:
        result = COMMAND_PROCESSOR::ExecutePacketType3_INDIRECT_BUFFER(packet, count);
        break;
      case PM4_WAIT_REG_MEM:
        result = COMMAND_PROCESSOR::ExecutePacketType3_WAIT_REG_MEM(packet, count);
        break;
      case PM4_REG_RMW:
        result = COMMAND_PROCESSOR::ExecutePacketType3_REG_RMW(packet, count);
        break;
      case PM4_REG_TO_MEM:
        result = COMMAND_PROCESSOR::ExecutePacketType3_REG_TO_MEM(packet, count);
        break;
      case PM4_MEM_WRITE:
        result = COMMAND_PROCESSOR::ExecutePacketType3_MEM_WRITE(packet, count);
        break;
      case PM4_COND_WRITE:
        result = COMMAND_PROCESSOR::ExecutePacketType3_COND_WRITE(packet, count);
        break;
      case PM4_EVENT_WRITE:
        result = COMMAND_PROCESSOR::ExecutePacketType3_EVENT_WRITE(packet, count);
        break;
      case PM4_EVENT_WRITE_SHD:
        result = COMMAND_PROCESSOR::ExecutePacketType3_EVENT_WRITE_SHD(packet, count);
        break;
      case PM4_EVENT_WRITE_EXT:
        result = COMMAND_PROCESSOR::ExecutePacketType3_EVENT_WRITE_EXT(packet, count);
        break;
      case PM4_EVENT_WRITE_ZPD:
        result = COMMAND_PROCESSOR::ExecutePacketType3_EVENT_WRITE_ZPD(packet, count);
        break;
      case PM4_DRAW_INDX:
        result = COMMAND_PROCESSOR::ExecutePacketType3_DRAW_INDX(packet, count);
        break;
      case PM4_DRAW_INDX_2:
        result = COMMAND_PROCESSOR::ExecutePacketType3_DRAW_INDX_2(packet, count);
        break;
      case PM4_SET_CONSTANT:
        result = COMMAND_PROCESSOR::ExecutePacketType3_SET_CONSTANT(packet, count);
        break;
      case PM4_SET_CONSTANT2:
        result = COMMAND_PROCESSOR::ExecutePacketType3_SET_CONSTANT2(packet, count);
        break;
      case PM4_LOAD_ALU_CONSTANT:
        result = COMMAND_PROCESSOR::ExecutePacketType3_LOAD_ALU_CONSTANT(packet, count);
        break;
      case PM4_SET_SHADER_CONSTANTS:
        result = COMMAND_PROCESSOR::ExecutePacketType3_SET_SHADER_CONSTANTS(packet, count);
        break;
      case PM4_IM_LOAD:
        result = COMMAND_PROCESSOR::ExecutePacketType3_IM_LOAD(packet, count);
        break;
      case PM4_IM_LOAD_IMMEDIATE:
        result = COMMAND_PROCESSOR::ExecutePacketType3_IM_LOAD_IMMEDIATE(packet, count);
        break;
      case PM4_INVALIDATE_STATE:
        result = COMMAND_PROCESSOR::ExecutePacketType3_INVALIDATE_STATE(packet, count);
        break;
      case PM4_VIZ_QUERY:
    ...
    trace_writer_.WritePacketEnd();
#if REX_ENABLE_TRACE_WRITER_INSTRUMENTATION == 1
    ...
    assert_true(reader_.read_offset() ==
                (data_start_offset + (count * sizeof(uint32_t))) % reader_.capacity());
    return result;
  } else {
...
REX_NOINLINE
REX_COLD
bool COMMAND_PROCESSOR::HitUnimplementedOpcode(uint32_t opcode, uint32_t count) REX_RESTRICT {
...
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_ME_INIT(uint32_t packet, uint32_t count) REX_RESTRICT {
...
bool COMMAND_PROCESSOR::ExecutePacketType3_NOP(uint32_t packet, uint32_t count) REX_RESTRICT {
...
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_INTERRUPT(uint32_t packet, uint32_t count) REX_RESTRICT {
  SCOPE_profile_cpu_f("gpu");
  ...
  for (int n = 0; n < 6; n++) {
    if (cpu_mask & (1 << n)) {
      // ReXGlue: graphics_system_ may be null in tests.
      if (graphics_system_) {
...
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_XE_SWAP(uint32_t packet, uint32_t count) REX_RESTRICT {
  SCOPE_profile_cpu_f("gpu");

  Profiler::Flip();
#ifdef REXGLUE_ENABLE_PERF_COUNTERS
  // ReXGlue: frame time / FPS counters for the debug overlay (read at the next Flip()).
  static uint64_t last_swap_tick = 0;
  uint64_t swap_tick = chrono::Clock::QueryHostTickCount();
  uint64_t swap_ticks = last_swap_tick ? swap_tick - last_swap_tick : 0;
  PROFILE_FRAME_TIME_US(int64_t(swap_ticks * 1000000 / chrono::Clock::QueryHostTickFrequency()));
  PROFILE_FPS(swap_ticks ? int64_t(chrono::Clock::QueryHostTickFrequency() / swap_ticks) : 0);
  last_swap_tick = swap_tick;
#endif
  ...
  COMMAND_PROCESSOR::IssueSwap(frontbuffer_ptr, frontbuffer_width, frontbuffer_height);

  ++counter_;
  return true;
}

bool COMMAND_PROCESSOR::ExecutePacketType3_INDIRECT_BUFFER(uint32_t packet,
                                                           uint32_t count) REX_RESTRICT {
...
static bool MatchValueAndRef(uint32_t value, uint32_t ref, uint32_t wait_info) {
  ...
  // throughput
  return ((((value < ref) << 1) | ((value <= ref) << 2) | ((value == ref) << 3) |
           ((value != ref) << 4) | ((value >= ref) << 5) | ((value > ref) << 6) | (1 << 7)) >>
          (wait_info & 7)) &
         1;
}
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_WAIT_REG_MEM(uint32_t packet,
                                                        uint32_t count) REX_RESTRICT {
  SCOPE_profile_cpu_f("gpu");
  ...
  // ReXGlue: out-of-range registers are read from extended_register_values_.
  const bool is_extended_register = !is_memory && poll_reg_addr >= RegisterFile::kRegisterCount;
  const volatile uint32_t& value_ref =
      is_memory
          ? *reinterpret_cast<uint32_t*>(memory_->TranslatePhysical(poll_reg_addr & ~uint32_t(0x3)))
          : register_file_->values[is_extended_register ? 0 : poll_reg_addr];

  bool matched = false;

  do {
    uint32_t value = is_extended_register ? COMMAND_PROCESSOR::ReadRegisterValue(poll_reg_addr)
                                          : uint32_t(value_ref);
    if (is_memory) {
...
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_REG_RMW(uint32_t packet, uint32_t count) REX_RESTRICT {
...
bool COMMAND_PROCESSOR::ExecutePacketType3_REG_TO_MEM(uint32_t packet,
                                                      uint32_t count) REX_RESTRICT {
  ...
  // ReXGlue: also covers extended (out-of-range) registers.
  uint32_t reg_val = COMMAND_PROCESSOR::ReadRegisterValue(reg_addr);

  auto endianness = static_cast<xenos::Endian>(mem_addr & 0x3);
  mem_addr &= ~0x3;
  reg_val = GpuSwap(reg_val, endianness);
  rex::store(memory_->TranslatePhysical(mem_addr), reg_val);
  trace_writer_.WriteMemoryWrite(CpuToGpu(mem_addr), 4);

  return true;
}
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_MEM_WRITE(uint32_t packet, uint32_t count) REX_RESTRICT {
  ...
  for (uint32_t i = 0; i < count - 1; i++) {
    ...
    rex::store(memory_->TranslatePhysical(addr), write_data);
...
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_COND_WRITE(uint32_t packet,
                                                      uint32_t count) REX_RESTRICT {
  ...
  if (wait_info & 0x10) {
    ...
    value = rex::load<uint32_t>(memory_->TranslatePhysical(poll_reg_addr));
    value = GpuSwap(value, endianness);
  } else {
    // Register.
    // ReXGlue: also covers extended (out-of-range) registers.
    value = COMMAND_PROCESSOR::ReadRegisterValue(poll_reg_addr);
  }
  ...
  if (matched) {
    ...
    if (wait_info & 0x100) {
      ...
      write_reg_addr &= ~0x3;
      write_data = GpuSwap(write_data, endianness);
      rex::store(memory_->TranslatePhysical(write_reg_addr), write_data);
...
REX_FORCEINLINE
void COMMAND_PROCESSOR::WriteEventInitiator(uint32_t value) REX_RESTRICT {
  register_file_->values[XE_GPU_REG_VGT_EVENT_INITIATOR] = value;
}
bool COMMAND_PROCESSOR::ExecutePacketType3_EVENT_WRITE(uint32_t packet,
                                                       uint32_t count) REX_RESTRICT {
...
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_EVENT_WRITE_SHD(uint32_t packet,
                                                           uint32_t count) REX_RESTRICT {
  ...
  rex::store(write_destination, data_value);
  trace_writer_.WriteMemoryWrite(CpuToGpu(address), 4);
  return true;
}

bool COMMAND_PROCESSOR::ExecutePacketType3_EVENT_WRITE_EXT(uint32_t packet,
                                                           uint32_t count) REX_RESTRICT {
  ...
  uint16_t extents[] = {
      ...
      byte_swap<unsigned short>(xenos::kTexture2DCubeMaxWidthHeight >> 3),  // max x
      byte_swap<unsigned short>(0 >> 3),                                    // min y
      byte_swap<unsigned short>(xenos::kTexture2DCubeMaxWidthHeight >> 3),  // max y
...
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_EVENT_WRITE_ZPD(uint32_t packet,
                                                           uint32_t count) REX_RESTRICT {
  assert_true(count == 1);
  uint32_t initiator = reader_.ReadAndSwap<uint32_t>();
  // Writeback initiator.
  ...
  if (GetZPDMode() != ZPDMode::kFake && !zpd_force_fake_fallback_) {
    ...
    if (!logical_active && is_end_record) {
      ...
      if (GetZPDMode() == ZPDMode::kFast || GetZPDMode() == ZPDMode::kFastAlt) {
        ...
        COMMAND_PROCESSOR::WriteZPDReport(0, report_record_base, 0, cached_delta, false);
      } else {
...
bool COMMAND_PROCESSOR::ExecutePacketType3Draw(uint32_t packet, const char* opcode_name,
                                               uint32_t viz_query_condition,
                                               uint32_t count_remaining) REX_RESTRICT {
  ...
  register_file_->values[XE_GPU_REG_VGT_DRAW_INITIATOR] = vgt_draw_initiator.value;
  ...
  if (draw_succeeded) {
    const auto viz_decision = COMMAND_PROCESSOR::GetVIZDecision(viz_query_condition);
    ...
    if (!viz_decision.draw || viz_decision.use_predicate) {
      ...
      const auto shader_may_memexport = [](const Shader* shader) {
        return shader && (!shader->is_ucode_analyzed() || shader->memexport_eM_written());
      };
      viz_token_has_memexport =
          shader_may_memexport(active_vertex_shader_) || shader_may_memexport(active_pixel_shader_);
    ...
    if (viz_decision.draw || viz_token_has_memexport || viz_copy_passthrough) {
      IndexBufferInfo* draw_index_buffer_info = is_indexed ? &index_buffer_info : nullptr;
      const bool major_mode_explicit =
          xenos::IsMajorModeExplicit(vgt_draw_initiator.major_mode, vgt_draw_initiator.prim_type);
      const bool use_viz_predicate =
          viz_decision.use_predicate && !viz_token_has_memexport && !viz_copy_passthrough;
      ...
      if (const auto viz_query = register_file_->Get<reg::PA_SC_VIZ_QUERY>();
        ...
        // always was.
        if (REXCVAR_GET(viz_query)) {
          CommandProcessor::VIZQueryDrawResult query_draw_result =
              CommandProcessor::VIZQueryDrawResult::kFallback;
          if (!COMMAND_PROCESSOR::IssueDraw(vgt_draw_initiator.prim_type,
                                            vgt_draw_initiator.num_indices, draw_index_buffer_info,
                                            major_mode_explicit, &query_draw_result)) {
            query_draw_result = CommandProcessor::VIZQueryDrawResult::kFailed;
          }
          if (query_draw_result == CommandProcessor::VIZQueryDrawResult::kFailed) {
            draw_succeeded = false;
            REXGPU_ERROR("{}({}, {}, {}): Failed in backend VIZ query draw", opcode_name,
                         uint32_t(vgt_draw_initiator.num_indices),
                         uint32_t(vgt_draw_initiator.prim_type),
                         uint32_t(vgt_draw_initiator.source_select));
          } else if (query_draw_result == CommandProcessor::VIZQueryDrawResult::kDrawn) {
            COMMAND_PROCESSOR::NoteVIZDraw(true);
          }
        }
      } else {
        draw_succeeded = COMMAND_PROCESSOR::IssueDraw(vgt_draw_initiator.prim_type,
                                                      vgt_draw_initiator.num_indices,
                                                      draw_index_buffer_info, major_mode_explicit);
        if (REXCVAR_GET(viz_query) && draw_succeeded) {
          COMMAND_PROCESSOR::NoteVIZDraw(true);
        }
        if (!draw_succeeded) {
...
bool COMMAND_PROCESSOR::ExecutePacketType3_DRAW_INDX(uint32_t packet, uint32_t count) REX_RESTRICT {
  ...
  --count_remaining;
  return COMMAND_PROCESSOR::ExecutePacketType3Draw(packet, "PM4_DRAW_INDX", viz_query_condition,
                                                   count_remaining);
}

bool COMMAND_PROCESSOR::ExecutePacketType3_DRAW_INDX_2(uint32_t packet,
                                                       uint32_t count) REX_RESTRICT {
  // "draw using supplied indices in packet"
  // Generally used by Xbox 360 Direct3D 9 for kAutoIndex source.
  // No viz query token.
  return COMMAND_PROCESSOR::ExecutePacketType3Draw(packet, "PM4_DRAW_INDX_2", 0, count);
}
REX_FORCEINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_SET_CONSTANT(uint32_t packet,
                                                        uint32_t count) REX_RESTRICT {
...
REX_NOINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_SET_CONSTANT2(uint32_t packet,
                                                         uint32_t count) REX_RESTRICT {
  uint32_t offset_type = reader_.ReadAndSwap<uint32_t>();
...
REX_FORCEINLINE
bool COMMAND_PROCESSOR::ExecutePacketType3_LOAD_ALU_CONSTANT(uint32_t packet,
                                                             uint32_t count) REX_RESTRICT {
  ...
  switch (type) {
    ...
    case 1:  // FETCH
      trace_writer_.WriteMemoryRead(CpuToGpu(address), size_dwords * 4);
      COMMAND_PROCESSOR::WriteFetchRangeFromMem(index, xlat_address, size_dwords);
      break;
    case 2:  // BOOL
      trace_writer_.WriteMemoryRead(CpuToGpu(address), size_dwords * 4);

      COMMAND_PROCESSOR::WriteBoolRangeFromMem(index, xlat_address, size_dwords);
      break;
    case 3:  // LOOP
      trace_writer_.WriteMemoryRead(CpuToGpu(address), size_dwords * 4);

      COMMAND_PROCESSOR::WriteLoopRangeFromMem(index, xlat_address, size_dwords);
    ...
    case 4:  // REGISTERS
      ...
      COMMAND_PROCESSOR::WriteREGISTERSRangeFromMem(index, xlat_address, size_dwords);
      break;
    default:
      assert_always();
      return true;
...
bool COMMAND_PROCESSOR::ExecutePacketType3_SET_SHADER_CONSTANTS(uint32_t packet,
                                                                uint32_t count) REX_RESTRICT {
  uint32_t offset_type = reader_.ReadAndSwap<uint32_t>();
  uint32_t index = offset_type & 0xFFFF;
  uint32_t countm1 = count - 1;
...
bool COMMAND_PROCESSOR::ExecutePacketType3_IM_LOAD(uint32_t packet, uint32_t count) REX_RESTRICT {
  SCOPE_profile_cpu_f("gpu");
  ...
  auto shader = COMMAND_PROCESSOR::LoadShader(
      shader_type, addr, memory_->TranslatePhysical<uint32_t*>(addr), size_dwords);
  switch (shader_type) {
...
bool COMMAND_PROCESSOR::ExecutePacketType3_IM_LOAD_IMMEDIATE(uint32_t packet,
                                                             uint32_t count) REX_RESTRICT {
  SCOPE_profile_cpu_f("gpu");
  ...
  auto shader =
      COMMAND_PROCESSOR::LoadShader(shader_type, uint32_t(reader_.read_ptr()),
...
bool COMMAND_PROCESSOR::ExecutePacketType3_INVALIDATE_STATE(uint32_t packet,
                                                            uint32_t count) REX_RESTRICT {
...
bool COMMAND_PROCESSOR::ExecutePacketType3_VIZ_QUERY(uint32_t packet, uint32_t count) REX_RESTRICT {
...
// ReXGlue: clang requires REX_RESTRICT to match the declaration.
uint32_t COMMAND_PROCESSOR::ExecutePrimaryBuffer(uint32_t read_index,
                                                 uint32_t write_index) REX_RESTRICT {
  SCOPE_profile_cpu_f("gpu");
#if REX_ENABLE_TRACE_WRITER_INSTRUMENTATION == 1
  ...
  RingBuffer old_reader = reader_;
  new (&reader_) RingBuffer(memory_->TranslatePhysical(primary_buffer_ptr_), primary_buffer_size_);
  ...
  // it likely is already in L3 cache, but in a zen system it may be another
  // chiplets l3
  reader_.BeginPrefetchedRead<swcache::PrefetchTag::Level2>(GetCurrentRingReadCount());
  do {
    if (!COMMAND_PROCESSOR::ExecutePacket()) {
