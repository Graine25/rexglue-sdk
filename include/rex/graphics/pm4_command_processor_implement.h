...
bool COMMAND_PROCESSOR::ExecutePacketType3_XE_SWAP(uint32_t packet,
  ...
  // rexglue: no tracy frame-marker in the plugin (perf/debug only).
  // Profiler::Flip();
...
bool COMMAND_PROCESSOR::ExecutePacketType3_EVENT_WRITE_SHD(
  ...
  // rexglue: Canary's MEM_WRITE writeback-region address remap
  // (XE_GPU_REG_WRITEBACK_START/SIZE) is not in ReXGlue's register set; dropped
  // (direct physical write, matching the pre-mixin behavior).
