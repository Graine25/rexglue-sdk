...
// ReXGlue: self-protecting. The scoped-enum backward-compat shim below only
// applies to glslang 16+ (spirv.hpp11 / scoped `enum class Op`). On the current
// pre-migration glslang, spv::OpXxx is already valid, spirv.hpp11 does not exist,
// and this header is INERT. When the glslang submodule is bumped to 16 the shim
// activates automatically -- no code change needed here.
#if __has_include(<SPIRV/spirv.hpp11>)

#include <SPIRV/spirv.hpp11>
#include <SPIRV/spvUtil.h>
// ReXGlue: pull glslang's SPIR-V builder/IR headers BEFORE the compatibility
// macros below, so this header is safe to include in any order (our clang-format
// regroups includes, which can otherwise place these macros ahead of glslang and
// corrupt its own `enum class Op`). Hand-maintained; do not re-translate.
...
#endif  // __has_include(<SPIRV/spirv.hpp11>)
