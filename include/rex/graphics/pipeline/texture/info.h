...
#define R(x, y) xenos::TextureFormat::x, xenos::TextureFormat::y
  constexpr xenos::TextureFormat entries[] = {R(k_16_EXPAND, k_16_FLOAT),
...
inline xenos::TextureFormat GetBaseFormat(xenos::TextureFormat texture_format) {
  ...
  if ((indexer_table & format_mask)) {
    ...
    uint32_t sparse_index = rex::bit_count(trailing_bits);

    __m128i index_in_low = _mm_cvtsi32_si128(static_cast<int>(sparse_index) | 0x80808000);

    __m128i new_format_low = _mm_shuffle_epi8(
        _mm_setr_epi8(table[0], table[1], table[2], table[3], table[4], table[5], table[6],
                      table[7], table[8], table[9], table[10], table[11], table[12], 0, 0, 0),
        index_in_low);

    uint32_t prelaundered = static_cast<uint32_t>(_mm_cvtsi128_si32(new_format_low));
...
struct TextureInfo {
  ...
  const char* format_name() const { return FormatInfo::GetName(static_cast<uint32_t>(format)); }
