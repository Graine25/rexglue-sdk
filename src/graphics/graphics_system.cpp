...
X_STATUS GraphicsSystem::Setup(cpu::Processor* processor,
  ...
  frame_limiter_worker_thread_ =
      kernel::object_ref<kernel::XHostThread>(new kernel::XHostThread(
          ...
          [this]() {
            uint64_t normalized_framerate_limit =
                std::max<uint64_t>(0, REXCVAR_GET(framerate_limit));
    ...
    // Linux uses full sleep duration due to scheduler quantum issues
            ...
            while (frame_limiter_worker_running_) {
              ...
              // frame limiter thread.
              if (kernel_state_->title_id() == 0) {  // rexglue: is_title_open
  ...
  // rexglue: ReXGlue's XHostThread has no creator-process arg and no
  // set_can_debugger_suspend; the frame limiter is a plain host thread.
...
X_STATUS GraphicsSystem::SetupPresentation(
    ::rex::ui::WindowedAppContext* app_context) {
  // Record the presentation context; provider/presenter creation happens in the
  // deferred SetupGuestGpu (the backend builds the provider with presentation).
  app_context_ = app_context;
  pending_presentation_ = true;
  return X_STATUS_SUCCESS;
}
...
X_STATUS GraphicsSystem::SetupGuestGpu(
  ...
  // Canary's Setup is virtual: the backend override creates the provider (with
  // presentation if requested), then the base wires the command processor, MMIO
  // register handlers and the frame-limiter/vblank threads.
  return Setup(function_dispatcher, kernel_state, app_context_,
               pending_presentation_);
}

void GraphicsSystem::DispatchInterruptCallback(uint32_t source, uint32_t cpu) {
  // rexglue: raise the guest GPU interrupt through the FunctionDispatcher rather
  // than Canary's KernelState::EmulateCPInterruptDPC.
  if (!interrupt_callback_) {
    return;
  }
  auto thread = kernel::XThread::GetCurrentThread();
  assert_not_null(thread);
  // Pick a CPU if unspecified (guess 2, as Xenia does).
  if (cpu == 0xFFFFFFFF) {
    cpu = 2;
  }
...
std::pair<uint32_t, uint32_t> GraphicsSystem::GetResolution() const {
  ...
  // rexglue: Canary reads the guest AV composite screen size from xconfig;
  // ReXGlue's KernelState exposes no xconfig, so default to 720p.
  // TODO: wire to the guest video-mode config.
  return {1280, 720};
}
