...
int shader_compiler_main(const std::vector<std::string>& args) {
  ...
  if (!REXCVAR_GET(shader_input_type).empty()) {
    if (REXCVAR_GET(shader_input_type) == "vs") {
      shader_type = xenos::ShaderType::kVertex;
    } else if (REXCVAR_GET(shader_input_type) == "ps") {
    ...
    if (REXCVAR_GET(shader_input).has_extension()) {
      auto extension = REXCVAR_GET(shader_input).extension();
      if (extension == ".vs") {
        shader_type = xenos::ShaderType::kVertex;
        valid_type = true;
  ...
  auto input_file = filesystem::OpenFile(REXCVAR_GET(shader_input), "rb");
  if (!input_file) {
    XELOGE("Unable to open input file: {}", REXCVAR_GET(shader_input));
    return 1;
  }
  size_t input_file_size = std::filesystem::file_size(REXCVAR_GET(shader_input));
  std::vector<uint32_t> ucode_dwords(input_file_size / 4);
  fread(ucode_dwords.data(), 4, ucode_dwords.size(), input_file);
  fclose(input_file);

  XELOGI("Opened {} as a {} shader, {} words ({} bytes).", REXCVAR_GET(shader_input),
  ...
  auto shader = std::make_unique<Shader>(
      ...
      REXCVAR_GET(shader_input_little_endian) ? std::endian::little
                                        : std::endian::big);
  ...
  if (REXCVAR_GET(shader_output_type) == "spirv" ||
      REXCVAR_GET(shader_output_type) == "spirvtext") {
    translator = std::make_unique<SpirvShaderTranslator>(
        spirv_features, true, true,
        REXCVAR_GET(shader_output_pixel_shader_interlock));
  } else if (REXCVAR_GET(shader_output_type) == "dxbc" ||
             REXCVAR_GET(shader_output_type) == "dxbctext") {
    translator = std::make_unique<DxbcShaderTranslator>(
        ui::GraphicsProvider::GpuVendorID(0),
        REXCVAR_GET(shader_output_bindless_resources),
        REXCVAR_GET(shader_output_pixel_shader_interlock));
  } else {
    // Just output microcode disassembly generated during microcode information
    // gathering.
    if (!REXCVAR_GET(shader_output).empty()) {
      auto output_file = filesystem::OpenFile(REXCVAR_GET(shader_output), "wb");
  ...
  if (shader_type == xenos::ShaderType::kVertex) {
    if (REXCVAR_GET(vertex_shader_output_type) == "linedomaincp") {
      host_vertex_shader_type =
          Shader::HostVertexShaderType::kLineDomainCPIndexed;
    } else if (REXCVAR_GET(vertex_shader_output_type) == "linedomainpatch") {
      host_vertex_shader_type =
          Shader::HostVertexShaderType::kLineDomainPatchIndexed;
    } else if (REXCVAR_GET(vertex_shader_output_type) == "triangledomaincp") {
      host_vertex_shader_type =
          Shader::HostVertexShaderType::kTriangleDomainCPIndexed;
    } else if (REXCVAR_GET(vertex_shader_output_type) == "triangledomainpatch") {
      host_vertex_shader_type =
          Shader::HostVertexShaderType::kTriangleDomainPatchIndexed;
    } else if (REXCVAR_GET(vertex_shader_output_type) == "quaddomaincp") {
      host_vertex_shader_type =
          Shader::HostVertexShaderType::kQuadDomainCPIndexed;
    } else if (REXCVAR_GET(vertex_shader_output_type) == "quaddomainpatch") {
      host_vertex_shader_type =
  ...
  if (REXCVAR_GET(shader_output_type) == "spirvtext") {
  ...
  if (REXCVAR_GET(shader_output_type) == "dxbctext") {
...
XE_DEFINE_CONSOLE_APP("xenia-gpu-shader-compiler",
                      rex::graphics::shader_compiler_main, "shader.bin",
                      "shader_input");
