...
constexpr unsigned int GetHighestRegisterNumber() {
  uint32_t highest = 0;
#define XE_GPU_REGISTER(index, type, name) highest = std::max<uint32_t>(highest, index);
...
#define XE_GPU_REGISTER(index, type, name) lowest = std::min<uint32_t>(lowest, index);
#include <rex/graphics/register_table.inc>
#undef XE_GPU_REGISTER
...
#define XE_GPU_REGISTER(index, type, name) \
  result[(index - lowest_register) / 64] |= 1ULL << ((index - lowest_register) % 64);
...
static constexpr ValidRegisterBitset valid_register_bitset = BuildValidRegisterBitset();
...
bool RegisterFile::IsValidRegister(uint32_t index) {
  if (REX_UNLIKELY(index < lowest_register) || REX_UNLIKELY(index > highest_register)) {
    return false;
  }
