...
bool VulkanZPDQueryPool::EnsureInitialized(const ui::vulkan::VulkanDevice* vulkan_device,
  ...
  auto initialize_fsi_counter_path = [&]() -> bool {
    ...
    dfn.vkGetBufferMemoryRequirements(device, fsi_counter_buffer_, &counter_mem_reqs);
    ...
    fsi_counter_readback_is_coherent_ = (vulkan_device_->memory_types().host_coherent &
                                         (1u << readback_alloc_info.memoryTypeIndex)) != 0;
    ...
    fsi_counter_readback_mapping_ = reinterpret_cast<uint32_t*>(fsi_counter_readback_mapping);
...
void VulkanZPDQueryPool::Shutdown() {
  ...
  if (fsi_counter_readback_mapping_ && fsi_counter_readback_memory_ != VK_NULL_HANDLE) {
...
void VulkanZPDQueryPool::ClearFSICounter(DeferredCommandBuffer& deferred_command_buffer,
  ...
  deferred_command_buffer.CmdVkPipelineBarrier(VK_PIPELINE_STAGE_TRANSFER_BIT,
...
void VulkanZPDQueryPool::RecordResolveBatch(VkCommandBuffer command_buffer) {
  ...
  build_ranges(fsi_counter_resolve_batch_indices_, fsi_counter_resolve_batch_pending_);
  ...
  for (const ResolveRange& range : resolve_batch_ranges_) {
    ...
    uint32_t count = std::min(range.count, capacity_ - range.start);
    VkDeviceSize offset = static_cast<VkDeviceSize>(range.start) * sizeof(uint32_t);
    ...
    counter_barrier.srcAccessMask = VK_ACCESS_SHADER_WRITE_BIT | VK_ACCESS_TRANSFER_WRITE_BIT;
  ...
  if (!fsi_resolve_barrier_scratch_.empty()) {
    ...
    dfn.vkCmdCopyBuffer(command_buffer, fsi_counter_buffer_, fsi_counter_readback_buffer_,
                        uint32_t(fsi_resolve_copy_scratch_.size()),
                        fsi_resolve_copy_scratch_.data());
  }

  if (readback_barrier_offset == VK_WHOLE_SIZE || readback_barrier_end <= readback_barrier_offset) {
    return;
  }
...
void VulkanZPDQueryPool::InvalidateReadback() {
  if (vulkan_device_) {
    ...
    if (!readback_is_coherent_ && readback_memory_ != VK_NULL_HANDLE && readback_mapping_) {
    ...
    if (!fsi_counter_readback_is_coherent_ && fsi_counter_readback_memory_ != VK_NULL_HANDLE &&
