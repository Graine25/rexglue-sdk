...
std::string VulkanCommandProcessor::GetWindowTitleText() const {
  ...
  // rexglue: the plugin runtime has no emulator()/audio_system(); the audio
  // backend name is omitted from the window title (cosmetic).
  return title.str();
}
...
void VulkanCommandProcessor::UpdateSystemConstantValues(
  ...
  if (!edram_fragment_shader_interlock &&
      !REXCVAR_GET(gamma_render_target_as_unorm16)) {
    for (uint32_t i = 0; i < xenos::kMaxColorRenderTargets; ++i) {
