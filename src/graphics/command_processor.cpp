  ...
  worker_thread_ =
      kernel::object_ref<kernel::XHostThread>(new kernel::XHostThread(
          ...
          [this]() {
            WorkerThreadMain();
            return 0;
          }));  // rexglue: ReXGlue's XHostThread has no creator-process arg
...
XE_NOINLINE XE_COLD void CommandProcessor::LogKickoffInitator(uint32_t value) {
  // rexglue: Canary logs a guest pseudo-stacktrace of the ring-buffer kickoff
  // initiator here via Processor->backend()->PopulatePseudoStacktrace(). ReXGlue
  // drives the guest with a FunctionDispatcher (no Processor/backend stacktrace
  // API), so this debug-only trace is unavailable. No-op.
  (void)value;
}

void CommandProcessor::UpdateWritePointer(uint32_t value) {
  XE_UNLIKELY_IF(REXCVAR_GET(log_ringbuffer_kickoff_initiator_bts)) {
    LogKickoffInitator(value);
  }
  write_ptr_index_ = value;
  write_ptr_index_event_->Set();  // rexglue: Xenia's SetBoostPriority -> Set()
}
