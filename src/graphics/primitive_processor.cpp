...
REXCVAR_DEFINE_BOOL(force_convert_triangle_fans_to_lists, false, "GPU.Debug",
...
REXCVAR_DEFINE_BOOL(force_convert_line_loops_to_strips, false, "GPU.Debug",
                    "For host graphics API downlevel support testing only, force CPU "
                    "conversion of line loops to line strips even if the host supports line "
                    "loop primitives natively.");
REXCVAR_DEFINE_BOOL(force_convert_quad_lists_to_triangle_lists, false, "GPU.Debug",
                    "For host graphics API downlevel support testing only, force CPU "
...
REXCVAR_DEFINE_BOOL(ignore_32bit_vertex_index_support, false, "GPU.Debug",
...
REXCVAR_DEFINE_INT32(primitive_processor_cache_min_indices, 4096, "GPU.Debug",
...
bool PrimitiveProcessor::Process(ProcessingResult& result_out) {
  ...
  if (vgt_draw_initiator.source_select == xenos::SourceSelect::kDMA &&
    ...
    REXGPU_WARN(
        ...
        guest_draw_vertex_count, uint32_t(vgt_dma_size.num_words));
    guest_draw_vertex_count = vgt_dma_size.num_words;
  }
  ...
  if (host_vertex_shader_type == Shader::HostVertexShaderType::kPointListAsTriangleStrip ||
    ...
    if (host_primitive_type != guest_primitive_type) {
      ...
      // ReXGlue (MoltenVK, no fullDrawIndexUint32): pre-convert 32-bit guest DMA
      // indices to host-endian 24-bit for non-kVertex host vertex shader types.
...
PrimitiveProcessor::CacheTransaction::CacheTransaction(PrimitiveProcessor& processor, CacheKey key)
    : function_dispatcher_(processor), key_(key) {
  assert_zero(function_dispatcher_.cache_currently_processing_size_bytes_);
  ...
  uint32_t size_bytes =
      ...
      key_.count;
  {
    auto global_lock = function_dispatcher_.global_critical_region_.Acquire();
    auto cache_map_it = function_dispatcher_.cache_map_.find(key_);
    if (cache_map_it != function_dispatcher_.cache_map_.end()) {
      result_ = function_dispatcher_.cache_entry_pool_[cache_map_it->second].result;
      ...
      function_dispatcher_.cache_currently_processing_base_ = key_.base;
      function_dispatcher_.cache_currently_processing_size_bytes_ = size_bytes;
    }
  }
  if (result_type_ != ResultType::kExisting) {
    ...
    if (!function_dispatcher_.memory_invalidation_callback_handle_) {
      function_dispatcher_.memory_invalidation_callback_handle_ =
          function_dispatcher_.memory_.RegisterPhysicalMemoryInvalidationCallback(
              MemoryInvalidationCallbackThunk, &function_dispatcher_);
    }
    function_dispatcher_.memory_.EnablePhysicalMemoryAccessCallbacks(key_.base, size_bytes, true,
                                                                     false);
  }
}
...
PrimitiveProcessor::CacheTransaction::~CacheTransaction() {
  ...
  function_dispatcher_.cache_currently_processing_base_ = 0;
  function_dispatcher_.cache_currently_processing_size_bytes_ = 0;

  if (result_type_ == ResultType::kNewSet) {
    size_t new_entry_index;
    if (function_dispatcher_.cache_bucket_free_first_entry_ != SIZE_MAX) {
      new_entry_index = function_dispatcher_.cache_bucket_free_first_entry_;
      function_dispatcher_.cache_bucket_free_first_entry_ =
          function_dispatcher_.cache_entry_pool_[new_entry_index].free_next;
    } else {
      new_entry_index = function_dispatcher_.cache_entry_pool_.size();
      function_dispatcher_.cache_entry_pool_.emplace_back();
    }
    CacheEntry& new_entry = function_dispatcher_.cache_entry_pool_[new_entry_index];
    ...
    for (uint32_t link_index = 0; link_index < bucket_count; ++link_index) {
      ...
      uint64_t& bucket_non_empty_l1_ref =
          function_dispatcher_.cache_buckets_non_empty_l1_[bucket_index >> 6];
      uint64_t bucket_non_empty_l1_bit = uint64_t(1) << (bucket_index & 63);
      size_t& bucket_first_entry_ref =
          function_dispatcher_.cache_bucket_first_entries_[bucket_index];
      if (bucket_non_empty_l1_ref & bucket_non_empty_l1_bit) {
        ...
        CacheEntry& bucket_first_entry =
            function_dispatcher_.cache_entry_pool_[bucket_first_entry_ref];
        ...
        function_dispatcher_.UpdateCacheBucketsNonEmptyL2(bucket_index >> 6, global_lock);
      }
    ...
    function_dispatcher_.cache_map_.emplace(key_, new_entry_index);
  }
}
