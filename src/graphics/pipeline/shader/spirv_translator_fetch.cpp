  ...
  if (instr.opcode == ucode::FetchOpcode::kGetTextureGradients) {
    ...
    if (size_needed_components) {
        ...
        case xenos::FetchOpDimension::kCube: {
          ...
          if (instr.dimension == xenos::FetchOpDimension::k1D) {
            ...
            spv::Id width_1d_minus_1 = builder_->createTriOp(
                spv::OpBitFieldUExtract, type_uint_, fetch_constant_word_2, const_uint_0_,
                builder_->makeUintConstant(xenos::kTexture1DMaxWidthLog2));
            size[0] = builder_->createTriOp(spv::OpSelect, type_uint_, data_is_1d, width_1d_minus_1,
                                            size[0]);
            size[1] = builder_->createTriOp(spv::OpSelect, type_uint_, data_is_1d, const_uint_0_,
                                            size[1]);
      ...
      if (selected_mip_grid_possible) {
        ...
        spv::Id fetch_constant_word_4_mips = builder_->createLoad(
            builder_->createAccessChain(spv::StorageClassUniform, uniform_fetch_constants_,
                                        id_vector_temp_),
            spv::NoPrecision);
        selected_mip_level =
            builder_->createTriOp(spv::OpBitFieldUExtract, type_uint_, fetch_constant_word_4_mips,
        ...
        selected_mip_locked =
            builder_->createBinOp(spv::OpIEqual, type_bool_, selected_mip_level, mip_max_level);
      }
      {
    ...
    if (REXCVAR_GET(draw_resolution_scaled_texture_offsets) &&
      ...
      is_texture_resolved = builder_->createBinOp(
          ...
          builder_->createBinOp(spv::OpBitwiseAnd, type_uint_, textures_resolved,
    ...
    if (is_texture_resolved != spv::NoResult && size_needed_components) {
      if (size[0] != spv::NoResult && draw_resolution_scale_x_ > 1) {
        ...
        size[0] = builder_->createTriOp(spv::OpSelect, type_float_, is_texture_resolved,
                                        scaled_size_x, size[0]);
      }
    ...
    uint32_t coordinates_needed_components =
        instr.opcode == ucode::FetchOpcode::kGetTextureWeights
            ...
            : ((UINT32_C(1) << xenos::GetFetchOpDimensionComponentCount(coordinate_dimension)) - 1);
    ...
    if (instr.opcode == ucode::FetchOpcode::kGetTextureWeights) {
      ...
      for (uint32_t i = 0; i <= uint32_t(coordinate_dimension != xenos::FetchOpDimension::k1D);
        ...
        if (instr.attributes.unnormalized_coordinates) {
          ...
          if (is_texture_resolved != spv::NoResult && ((i == 0 && draw_resolution_scale_x_ > 1) ||
            ...
            float scale =
                (i == 0) ? float(draw_resolution_scale_x_) : float(draw_resolution_scale_y_);
            spv::Id scaled_coord = builder_->createNoContractionBinOp(
                spv::OpFMul, type_float_, coordinate_ref, builder_->makeFloatConstant(scale));
            coordinate_ref = builder_->createTriOp(spv::OpSelect, type_float_, is_texture_resolved,
                                                   scaled_coord, coordinate_ref);
          }
      ...
      if (instr.dimension == xenos::FetchOpDimension::k1D &&
        ...
        id_vector_temp_.push_back(
            builder_->makeIntConstant(int((fetch_constant_word_0_index + 5) & 3)));
        spv::Id fetch_constant_word_5_for_1d = builder_->createLoad(
            builder_->createAccessChain(spv::StorageClassUniform, uniform_fetch_constants_,
                                        id_vector_temp_),
            spv::NoPrecision);
        spv::Id data_dimension_1d =
            builder_->createTriOp(spv::OpBitFieldUExtract, type_uint_, fetch_constant_word_5_for_1d,
                                  builder_->makeUintConstant(9), builder_->makeUintConstant(2));
        ...
        spv::Id is_wide = builder_->createBinOp(spv::OpUGreaterThan, type_bool_,
                                                size_1d_width_minus_1_uint, max_width_minus_1);
        spv::Id is_wide_1d =
        ...
        spv::Id coord_x_wide, coord_y_wide;
        {
          ...
          spv::Id num_rows = builder_->createBinBuiltinCall(
              ...
              builder_->createUnaryBuiltinCall(
                  ...
                  builder_->createNoContractionBinOp(spv::OpFDiv, type_float_, original_width_float,
                                                     row_width_float)),
        ...
        coordinates[0] = if_wide_1d.createMergePhi(coord_x_wide, coordinates[0]);
        coordinates[1] = if_wide_1d.createMergePhi(coord_y_wide, coordinates[1]);
      }
      if (instr.dimension == xenos::FetchOpDimension::k3DOrStacked) {
      ...
      if (instr.opcode == ucode::FetchOpcode::kGetTextureComputedLod) {
        ...
        if (use_lod_bias) {
          ...
          spv::Id grad_exp_adjust_max = builder_->createBinBuiltinCall(
              ...
              builder_->createTriOp(spv::OpBitFieldSExtract, type_int_,
                                    fetch_constant_word_4_signed, builder_->makeUintConstant(22),
                                    builder_->makeUintConstant(5)),
              builder_->createTriOp(spv::OpBitFieldSExtract, type_int_,
                                    fetch_constant_word_4_signed, builder_->makeUintConstant(27),
                                    builder_->makeUintConstant(5)));
          lod = builder_->createNoContractionBinOp(
              spv::OpFAdd, type_float_, lod,
        ...
        if (use_computed_lod && !use_lod_bias) {
          ...
          spv::Id lod_gradient_scale_h = builder_->createUnaryBuiltinCall(
              ...
              builder_->createNoContractionBinOp(spv::OpFAdd, type_float_, lod, grad_exp_adjust_h));
          spv::Id lod_gradient_scale_v = builder_->createUnaryBuiltinCall(
              type_float_, ext_inst_glsl_std_450_, GLSLstd450Exp2,
              builder_->createNoContractionBinOp(spv::OpFAdd, type_float_, lod, grad_exp_adjust_v));
          switch (coordinate_dimension) {
            case xenos::FetchOpDimension::k1D: {
              ...
              gradient_h_y =
                  builder_->createUnaryOp(spv::OpDPdxCoarse, type_float_, coordinates[1]);
              gradient_v_y =
                  builder_->createUnaryOp(spv::OpDPdyCoarse, type_float_, coordinates[1]);
              gradient_h_x = builder_->createNoContractionBinOp(spv::OpFMul, type_float_,
                                                                gradient_h_x, lod_gradient_scale_h);
              gradient_v_x = builder_->createNoContractionBinOp(spv::OpFMul, type_float_,
                                                                gradient_v_x, lod_gradient_scale_v);
              gradient_h_y = builder_->createNoContractionBinOp(spv::OpFMul, type_float_,
            ...
            case xenos::FetchOpDimension::k2D: {
              ...
              gradients_h = builder_->createNoContractionBinOp(
                  spv::OpVectorTimesScalar, type_float2_, gradients_h, lod_gradient_scale_h);
              gradients_v = builder_->createNoContractionBinOp(
                  spv::OpVectorTimesScalar, type_float2_, gradients_v, lod_gradient_scale_v);
            } break;
            case xenos::FetchOpDimension::k3DOrStacked: {
              if (instr.attributes.use_register_gradients) {
              ...
              gradients_h = builder_->createNoContractionBinOp(
                  spv::OpVectorTimesScalar, type_float3_, gradients_h, lod_gradient_scale_h);
              gradients_v = builder_->createNoContractionBinOp(
            ...
            builder_->createCompositeConstruct(type_float4_, id_vector_temp_);
        {
          ...
          spv::Id integer_scale_result_converted;
          {
            spv::Id normalized = builder_->createBinOp(
                ...
                builder_->makeUintConstant(0));
            SpirvBuilder::IfBuilder if_normalized(normalized, spv::SelectionControlMaskNone,
                                                  *builder_);
            ...
            normalized_result = builder_->createUnaryBuiltinCall(
                type_float4_, ext_inst_glsl_std_450_, GLSLstd450RoundEven, normalized_result);
            ...
            spv::Id scale_bits = builder_->createBinOp(
                ...
                builder_->smearScalar(spv::NoPrecision, integer_scale_bits_packed, type_uint4_),
            ...
            scale_bits = builder_->createBinOp(
                ...
                builder_->smearScalar(spv::NoPrecision, builder_->makeUintConstant(0x3F),
                                      type_uint4_));
            ...
            scale_shift = builder_->createBinOp(
                ...
                builder_->createBinOp(
                    ...
                    builder_->createBinOp(
                        ...
                        builder_->smearScalar(spv::NoPrecision, builder_->makeUintConstant(4),
                                              type_uint4_)),
                    const_uint4_1));
            spv::Id scale_uint =
                builder_->createBinOp(spv::OpISub, type_uint4_,
            ...
            spv::Id biased = builder_->createBinOp(
                ...
                builder_->createBinOp(
                    ...
                    builder_->createBinOp(
                        spv::OpShiftRightLogical, type_uint4_, scale_bits,
                        builder_->smearScalar(spv::NoPrecision, builder_->makeUintConstant(5),
                                              type_uint4_)),
                    const_uint4_1),
                const_uint4_0_);
            spv::Id scale_float = builder_->createTriOp(
                spv::OpSelect, type_float4_, biased,
                builder_->smearScalar(spv::NoPrecision, builder_->makeFloatConstant(0.5f),
                                      type_float4_),
                const_float4_1_);
            spv::Id scaled_result = builder_->createNoContractionBinOp(
                ...
                builder_->createNoContractionBinOp(
                    ...
                    builder_->createNoContractionBinOp(
                        ...
                        scale_float)),
                builder_->createNoContractionBinOp(spv::OpFSub, type_float4_, scale_float,
                                                   const_float4_1_));
