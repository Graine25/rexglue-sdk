...
void SpirvShaderTranslator::Reset() {
  ...
  var_main_rectangle_clip_distances_.fill(spv::NoResult);
  var_main_rectangle_cull_distances_.fill(spv::NoResult);
  var_main_rectangle_interpolators_.fill(spv::NoResult);
...
void SpirvShaderTranslator::StartTranslation() {
  ...
  if (!is_depth_only_fragment_shader_) {
    ...
    // ReXGlue: rectangle-list vertex loop - outputs of each guest vertex of the rectangle.
    if (IsSpirvRectangleListVertexLoopEnabled()) {
      ...
      // Plain arrays, not the gl_PerVertex block type (SPIRV-Cross rejects it in
      // Function storage).
  ...
  // ReXGlue: rectangle-list vertex loop - exits converge in the body, before the next guest vertex.
...
std::vector<uint8_t> SpirvShaderTranslator::CompleteTranslation() {
  ...
  if (!is_depth_only_fragment_shader_) {
    ...
    // ReXGlue: rectangle-list vertex loop - next guest vertex or exit (adds a pc phi operand).
    if (rectangle_vertex_loop) {
      function_main_->addBlock(main_loop_merge_);
      builder_->setBuildPoint(main_loop_merge_);
      CompleteRectangleListVertexLoopIteration();
    }
  ...
  if (is_vertex_shader()) {
    // ReXGlue: already done per guest vertex in the rectangle-list vertex loop.
    if (!rectangle_vertex_loop) {
...
void SpirvShaderTranslator::StartVertexOrTessEvalShaderBeforeMain() {
  ...
  if (IsSpirvTessEvalShader()) {
    ...
    builder_->addDecoration(input_tess_coord_, spv::DecorationBuiltIn,
                            static_cast<int>(spv::BuiltInTessCoord));
    ...
    builder_->addDecoration(input_vertex_index_, spv::DecorationBuiltIn,
                            static_cast<int>(spv::BuiltInVertexIndex));
  ...
  if (user_clip_plane_count > 0) {
    ...
    spv::Id type_clip_plane_constants =
        builder_->makeStructType(clip_plane_struct_members, "XeClipPlaneConstants");
    builder_->addMemberDecoration(type_clip_plane_constants, 0, spv::DecorationOffset, 0);
    builder_->addDecoration(type_clip_plane_constants, spv::DecorationBlock);
    uniform_clip_plane_constants_ =
        builder_->createVariable(spv::NoPrecision, spv::StorageClassUniform,
                                 type_clip_plane_constants, "xe_uniform_clip_planes");
    builder_->addDecoration(uniform_clip_plane_constants_, spv::DecorationDescriptorSet,
                            int(kDescriptorSetConstants));
    builder_->addDecoration(uniform_clip_plane_constants_, spv::DecorationBinding,
...
void SpirvShaderTranslator::StartVertexOrTessEvalShaderInMain() {
  ...
  // ReXGlue: the rectangle-list vertex loop is driven by CompleteRectangleListVertexLoopIteration.
  ...
  if (register_count()) {
    if (IsSpirvTessEvalShader()) {
      ...
      if (host_vertex_shader_type == Shader::HostVertexShaderType::kPointListAsTriangleStrip ||
        ...
        // ReXGlue: 3 guest vertices per rectangle in the rectangle-list vertex loop.
        if (host_vertex_shader_type ==
            Shader::HostVertexShaderType::kRectangleListAsTriangleStrip) {
          assert_true(IsSpirvRectangleListVertexLoopEnabled());
        ...
        vertex_index = LoadPrimitiveVertexIndexIfNeeded(vertex_index);
      } else {
      ...
      id_vector_temp_.push_back(const_int_0_);
      id_vector_temp_.push_back(const_int_0_);
      builder_->createStore(builder_->createUnaryOp(spv::OpConvertSToF, type_float_, vertex_index),
                            builder_->createAccessChain(spv::StorageClassFunction,
                                                        var_main_registers_, id_vector_temp_));
    }
  }
...
// ReXGlue: split out of StartVertexOrTessEvalShaderInMain for the rectangle-list vertex loop.
spv::Id SpirvShaderTranslator::LoadPrimitiveVertexIndexIfNeeded(spv::Id vertex_index) {
  spv::Id const_uint_2 = builder_->makeUintConstant(2);
  ...
  spv::Id load_vertex_index = builder_->createBinOp(
      ...
      const_uint_0_);
  SpirvBuilder::IfBuilder load_vertex_index_if(load_vertex_index,
  ...
  spv::Id loaded_vertex_index;
  {
    ...
    spv::Id vertex_index_address = builder_->createBinOp(
        ...
        builder_->createBinOp(
            ...
            builder_->createTriOp(spv::OpSelect, type_uint_, vertex_index_is_32bit, const_uint_2,
                                  builder_->makeUintConstant(1))));
    ...
    loaded_vertex_index = EndianSwap32Uint(
        loaded_vertex_index, builder_->createLoad(builder_->createAccessChain(
                                                      spv::StorageClassUniform,
                                                      uniform_system_constants_, id_vector_temp_),
                                                  spv::NoPrecision));
  }
  load_vertex_index_if.makeEndIf();
  // Select between the loaded index and the original index from Vulkan.
  return load_vertex_index_if.createMergePhi(loaded_vertex_index, vertex_index);
}

// ReXGlue: rectangle-list vertex loop.
void SpirvShaderTranslator::CompleteRectangleListVertexLoopIteration() {
  assert_true(IsSpirvRectangleListVertexLoopEnabled());
  assert_true(builder_->getBuildPoint() == main_loop_merge_);
  bool has_main_switch = !current_shader().label_addresses().empty();
  // For rectangle-list fallback without geometry shaders, run the guest vertex
  // shader 3 times (for all source vertices), then choose or synthesize the
  // output for the current host strip vertex.
  Modification shader_modification = GetSpirvShaderModification();
  uint32_t clip_distance_count = shader_modification.GetVertexClipDistanceCount();
  uint32_t cull_distance_count = shader_modification.GetVertexCullDistanceCount();
  ...
  auto store_rectangle_distance_members =
      [this, rectangle_guest_vertex_index_int](
        ...
        for (uint32_t i = 0; i < component_count; ++i) {
          ...
          id_vector_temp_.clear();
          id_vector_temp_.push_back(rectangle_guest_vertex_index_int);
          builder_->createStore(value, builder_->createAccessChain(spv::StorageClassFunction,
                                                                   variables[i], id_vector_temp_));
        }
      };
  if (clip_distance_count && output_per_vertex_clip_distance_member_index_ != 0) {
    store_rectangle_distance_members(output_per_vertex_clip_distance_member_index_,
                                     clip_distance_count, var_main_rectangle_clip_distances_);
  }
  if (cull_distance_count && output_per_vertex_cull_distance_member_index_ != 0) {
    store_rectangle_distance_members(output_per_vertex_cull_distance_member_index_,
      ...
      spv::SelectionControlDontFlattenMask, *builder_);
  {
    ...
    if (register_count()) {
      // Rectangle lists consume 3 guest vertices per primitive.
      ...
      rectangle_vertex_index = LoadPrimitiveVertexIndexIfNeeded(rectangle_vertex_index);
      // Convert the index to a signed integer and add the base to it.
      id_vector_temp_.clear();
      id_vector_temp_.push_back(builder_->makeIntConstant(kSystemConstantVertexBaseIndex));
      spv::Id rectangle_vertex_index_int = builder_->createBinOp(
          spv::OpIAdd, type_int_,
          builder_->createUnaryOp(spv::OpBitcast, type_int_, rectangle_vertex_index),
          builder_->createLoad(
              builder_->createAccessChain(spv::StorageClassUniform, uniform_system_constants_,
                                          id_vector_temp_),
              spv::NoPrecision));
      ...
      id_vector_temp_.push_back(const_int_0_);
      builder_->createStore(
          builder_->createUnaryOp(spv::OpConvertSToF, type_float_, rectangle_vertex_index_int),
  ...
  if_not_last_guest_vertex.makeBeginElse(false);
  {
    ...
    for (uint32_t i = 0; i < 3; ++i) {
      ...
      edge_lengths[i] = builder_->createBinOp(
          spv::OpFAdd, type_float_, builder_->createBinOp(spv::OpFMul, type_float_, edge_x, edge_x),
    ...
    for (uint32_t i = 1; i < 3; ++i) {
      spv::Id vertex_index_without_wrapping = builder_->createBinOp(
          spv::OpIAdd, type_int_, rectangle_vertex_indices[0], builder_->makeIntConstant(int(i)));
      rectangle_vertex_indices[i] =
          builder_->createTriOp(spv::OpSelect, type_int_,
    ...
    auto select_rectangle_vertex_component = [this, rectangle_host_vertex_is_0,
                                              rectangle_host_vertex_is_1](
                                                 spv::Id value_0, spv::Id value_1, spv::Id value_2,
                                                 spv::Id type) {
      return builder_->createTriOp(
          spv::OpSelect, type, rectangle_host_vertex_is_0, value_0,
          builder_->createTriOp(spv::OpSelect, type, rectangle_host_vertex_is_1, value_1, value_2));
    };
    ...
    id_vector_temp_.clear();
    id_vector_temp_.push_back(builder_->makeIntConstant(kOutputPerVertexMemberPosition));
    spv::Id rectangle_position_ptr =
        builder_->createAccessChain(spv::StorageClassOutput, output_per_vertex_, id_vector_temp_);
    spv::Id position_0 =
        load_rectangle_vertex_value(var_main_rectangle_per_vertex_, rectangle_vertex_indices[0]);
    spv::Id position_1 =
        load_rectangle_vertex_value(var_main_rectangle_per_vertex_, rectangle_vertex_indices[1]);
    spv::Id position_2 =
        load_rectangle_vertex_value(var_main_rectangle_per_vertex_, rectangle_vertex_indices[2]);
        ...
        [this, &select_rectangle_vertex_component, rectangle_host_vertex_is_synthetic,
         &load_rectangle_vertex_value, rectangle_vertex_indices](
          ...
          for (uint32_t i = 0; i < component_count; ++i) {
            ...
            id_vector_temp_.push_back(builder_->makeIntConstant(int(member_index)));
            id_vector_temp_.push_back(builder_->makeIntConstant(int(i)));
            builder_->createStore(builder_->createTriOp(spv::OpSelect, type_float_,
                                                        rectangle_host_vertex_is_synthetic,
                                                        value_synthetic, value_original),
    ...
    if (clip_distance_count && output_per_vertex_clip_distance_member_index_ != 0) {
      write_rectangle_distance_member(output_per_vertex_clip_distance_member_index_,
                                      clip_distance_count, var_main_rectangle_clip_distances_);
    }
    if (cull_distance_count && output_per_vertex_cull_distance_member_index_ != 0) {
      write_rectangle_distance_member(output_per_vertex_cull_distance_member_index_,
                                      cull_distance_count, var_main_rectangle_cull_distances_);
    }
  }
  if_not_last_guest_vertex.makeEndIf();
  // The last guest vertex has been processed - leave the main loop.
  if (!builder_->getBuildPoint()->isTerminated()) {
...
void SpirvShaderTranslator::CompleteVertexOrTessEvalShaderInMain() {
  ...
  if (user_clip_plane_count > 0) {
    ...
    for (uint32_t i = 0; i < user_clip_plane_count; ++i) {
      ...
      id_vector_temp_.clear();
      id_vector_temp_.push_back(builder_->makeIntConstant(0));       // Struct member 0
      id_vector_temp_.push_back(builder_->makeIntConstant(int(i)));  // Array index
      spv::Id clip_plane = builder_->createLoad(
          builder_->createAccessChain(spv::StorageClassUniform, uniform_clip_plane_constants_,
                                      id_vector_temp_),
          spv::NoPrecision);
...
void SpirvShaderTranslator::StartFragmentShaderBeforeMain() {
  ...
  if (edram_fragment_shader_interlock_) {
    ...
    builder_->addDecoration(type_zpd_fsi_counter, features_.spirv_version >= spv::Spv_1_3
                                                      ? spv::DecorationBlock
                                                      : spv::DecorationBufferBlock);
  ...
  if (edram_fragment_shader_interlock_ || DSV_IsApplyingPolygonOffset() ||
    ...
    builder_->addDecoration(input_front_facing_, spv::DecorationBuiltIn,
                            static_cast<int>(spv::BuiltInFrontFacing));
  ...
  if (edram_fragment_shader_interlock_) {
    ...
    builder_->addDecoration(input_sample_mask_, spv::DecorationBuiltIn,
                            static_cast<int>(spv::BuiltInSampleMask));
