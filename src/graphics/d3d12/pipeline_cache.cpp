...
bool PipelineCache::ReplaceShaderTranslationBinary(uint64_t ucode_hash, uint64_t modification,
  ...
  command_processor_.CallInThread([this, ucode_hash, modification, binary = std::move(binary)]() {
    ...
    // Mark every cached PSO that referenced this shader stale so it gets
    // rebuilt from the new binary on next use. Keep the objects resident
    // because async creation threads may still hold raw Pipeline pointers.
    current_pipeline_ = nullptr;
    for (auto it = pipelines_.begin(); it != pipelines_.end(); ++it) {
      ...
      if (desc.vertex_shader_hash == ucode_hash || desc.pixel_shader_hash == ucode_hash) {
        it->second->invalidated.store(true, std::memory_order_release);
        ID3D12PipelineState* state = it->second->state.exchange(nullptr, std::memory_order_acq_rel);
        if (state) {
          state->Release();
        }
      }
...
bool PipelineCache::ConfigurePipeline(
  ...
  if (current_pipeline_ != nullptr &&
      !std::memcmp(&current_pipeline_->description.description, &description,
                   sizeof(description)) &&
      !current_pipeline_->invalidated.load(std::memory_order_acquire)) {
  ...
  for (auto it = found_range.first; it != found_range.second; ++it) {
    Pipeline* found_pipeline = it->second;
    if (found_pipeline->invalidated.load(std::memory_order_acquire)) {
      continue;
    }
