...
REXCVAR_DEFINE_BOOL(d3d12_dxbc_disasm, false, "D3D12",
                    "Disassemble DXBC shaders after generation.");
REXCVAR_DEFINE_BOOL(d3d12_dxbc_disasm_dxilconv, false, "D3D12",
...
REXCVAR_DEFINE_INT32(d3d12_pipeline_creation_threads, -1, "D3D12",
...
REXCVAR_DEFINE_BOOL(d3d12_tessellation_wireframe, false, "D3D12",
...
void PipelineCache::InitializeShaderStorage(const std::filesystem::path& cache_root,
  ...
  pipeline_config.file_suffix = fmt::format(".{}.d3d12.xpso", edram_rov_used ? "rov" : "rtv");
  ...
  if (!storage_writer_.InitializeShaderStorage(
          ...
          [this,
           edram_rov_used](const std::set<std::pair<uint64_t, uint64_t>>& translations_needed) {
  ...
  if (!pipeline_stored_descriptions.empty()) {
    ...
    for (const PipelineStoredDescription& pipeline_stored_description :
      ...
      if (vertex_shader_it == shaders_.end()) {
        ...
        REXGPU_WARN("Pipeline cache: VS {:016X} not found in shader storage",
      ...
      if (pipeline_description.pixel_shader_hash) {
        ...
        if (pixel_shader_it == shaders_.end()) {
          ...
          REXGPU_WARN("Pipeline cache: PS {:016X} not found in shader storage",
      ...
      if (!pipeline_runtime_description.root_signature) {
        ...
        REXGPU_WARN("Pipeline cache: Root signature failed for VS {:016X} PS {:016X}",
                    pipeline_description.vertex_shader_hash,
                    pipeline_description.pixel_shader_hash);
        continue;
      }
      ...
      if (pixel_shader) {
        uint32_t bound_rts = (pipeline_description.render_targets[0].used ? 1 : 0) |
    ...
    if (!creation_threads_.empty()) {
      if (blocking) {
        ...
        if (creation_queue_.empty() && creation_threads_busy_ == 0) {
          ...
          completion_callback = nullptr;  // Prevent invocation at end of function
        }
      }
    }

    REXGPU_INFO("Pipeline cache loaded: {} created, {} already exist, {} total stored",
    ...
    if (pipelines_vs_not_found || pipelines_vs_translation_missing || pipelines_ps_not_found ||
        pipelines_ps_translation_missing || pipelines_root_sig_failed) {
      REXGPU_INFO(
    ...
    REXGPU_INFO("Pipeline creation took {} milliseconds",
...
DxbcShaderTranslator::Modification PipelineCache::GetCurrentPixelShaderModification(
    ...
    reg::RB_DEPTHCONTROL normalized_depth_control, bool apply_polygon_offset_in_shader) const {
...
bool PipelineCache::ConfigurePipeline(
    ...
    bool apply_polygon_offset_in_shader, uint32_t bound_depth_and_color_render_target_bits,
  ...
  if (!vertex_shader->is_translated() && !use_async) {
    ...
    if (storage_writer_.is_active() &&
        vertex_shader->shader().try_set_ucode_storage_index(storage_writer_.storage_index())) {
  ...
  if (pixel_shader != nullptr && !use_async) {
    ...
    if (!pixel_shader->is_translated()) {
      ...
      if (storage_writer_.is_active() &&
          pixel_shader->shader().try_set_ucode_storage_index(storage_writer_.storage_index())) {
        shader_storage_file_flush_needed_ = true;
  ...
  if (!GetCurrentStateDescription(
          ...
          runtime_description, use_async)) {
    return false;
  }
  PipelineDescription& description = runtime_description.description;

  if (current_pipeline_ != nullptr && current_pipeline_->description.description == description) {
...
void PipelineCache::TranslateShadersForStorage(
    const std::set<std::pair<uint64_t, uint64_t>>& translations_needed, bool edram_rov_used) {
  ...
  for (auto& shader_pair : shaders_) {
    ...
    if (translations_needed.lower_bound(std::make_pair(ucode_data_hash, uint64_t(0))) !=
        translations_needed.upper_bound(std::make_pair(ucode_data_hash, UINT64_MAX))) {
  ...
  auto translate_function = [&, this]() {
    ...
    if (REXCVAR_GET(d3d12_dxbc_disasm_dxilconv) && dxbc_converter_ && dxc_utils_ && dxc_compiler_) {
    ...
    while (true) {
      ...
      uint64_t ucode_data_hash = shader->ucode_data_hash();
      for (auto modification_it =
               translations_needed.lower_bound(std::make_pair(ucode_data_hash, uint64_t(0)));
  ...
  size_t thread_count =
      std::min(logical_processor_count - size_t(1), shaders_to_translate_list.size());
  ...
  REXGPU_INFO("Translated {} shaders in {} ms",
...
bool PipelineCache::GetCurrentStateDescription(
    ...
    const PrimitiveProcessor::ProcessingResult& primitive_processing_result,
    reg::RB_DEPTHCONTROL normalized_depth_control, uint32_t normalized_color_mask,
    bool depth_bias_in_pixel_shader, uint32_t bound_depth_and_color_render_target_bits,
  ...
  if (!edram_rov_used && !depth_bias_in_pixel_shader) {
    ...
    // creation.
    description_out.resolution_scale_native = uint32_t(render_target_cache_.IsDrawScaleNative());
  }
...
void PipelineCache::EnsurePipelineShadersTranslated(
    Pipeline* pipeline, DxbcShaderTranslator& translator, string::StringBuffer& ucode_disasm_buffer,
    IDxbcConverter* dxbc_converter, IDxcUtils* dxc_utils, IDxcCompiler* dxc_compiler,
    bool use_try_claim, bool handle_non_placeholder) {
  ...
  auto translate_shader = [&](D3D12Shader::D3D12Translation* translation, const char* shader_type) {
    if (!translation->is_translated()) {
      ...
      if (should_translate) {
        ...
        if (!TranslateAnalyzedShader(translator, *translation, dxbc_converter, dxc_utils,
                                     dxc_compiler)) {
          REXGPU_ERROR("Failed to translate {} shader {:016X}", shader_type,
                       shader.ucode_data_hash());
        } else {
          if (storage_writer_.is_active() &&
              shader.try_set_ucode_storage_index(storage_writer_.storage_index())) {
            shader_storage_file_flush_needed_ = true;
...
ID3D12PipelineState* PipelineCache::CreateD3D12Pipeline(
  ...
  if (!edram_rov_used) {
    ...
    const D3D12_BLEND kBlendFactorMap[] = {
        ...
        alpha_blend_factor_supported ? D3D12_BLEND_ALPHA_FACTOR : D3D12_BLEND_BLEND_FACTOR,
        alpha_blend_factor_supported ? D3D12_BLEND_INV_ALPHA_FACTOR : D3D12_BLEND_INV_BLEND_FACTOR,
    };
    ...
    for (uint32_t i = 0; i < xenos::kMaxColorRenderTargets; ++i) {
      ...
      if (rt.src_blend != PipelineBlendFactor::kOne ||
        ...
        if (blend_desc.BlendOp == D3D12_BLEND_OP_MIN || blend_desc.BlendOp == D3D12_BLEND_OP_MAX) {
  ...
  HRESULT hr = device->CreateGraphicsPipelineState(&state_desc, IID_PPV_ARGS(&state));
  if (FAILED(hr)) {
    if (runtime_description.pixel_shader != nullptr) {
      REXGPU_ERROR(
  ...
  if (runtime_description.pixel_shader != nullptr) {
    ...
    name =
        fmt::format(L"VS {:016X}", runtime_description.vertex_shader->shader().ucode_data_hash());
  }
...
void PipelineCache::CreationThread(size_t thread_index) {
  ...
  while (true) {
    ...
    EnsurePipelineShadersTranslated(pipeline_to_create, translator, ucode_disasm_buffer,
                                    dxbc_converter, dxc_utils, dxc_compiler,
                                    /*use_try_claim=*/true,
                                    /*handle_non_placeholder=*/true);

    // Create the D3D12 pipeline state object.
    ID3D12PipelineState* new_state = CreateD3D12Pipeline(pipeline_to_create->description);
    ...
    if (new_state != nullptr) {
      ...
      REXGPU_ERROR("Pipeline creation failed (VS {:016X}, PS {:016X})",
                   pipeline_to_create->description.vertex_shader
                       ? pipeline_to_create->description.vertex_shader->shader().ucode_data_hash()
                       : 0,
                   pipeline_to_create->description.pixel_shader
                       ? pipeline_to_create->description.pixel_shader->shader().ucode_data_hash()
                       : 0);
    }
...
void PipelineCache::CreateQueuedPipelinesOnProcessorThread() {
  ...
  while (true) {
    ...
    // Translate pending shaders and update root signature.
    EnsurePipelineShadersTranslated(pipeline_to_create, *shader_translator_, ucode_disasm_buffer_,
    ...
    if (new_state != nullptr) {
      ...
      REXGPU_WARN("ProcessorThread: Pipeline creation failed");
    }
  }
}
