...
#include <rex/ui/vulkan/gpu_completion_timeline.h>
...
VulkanGPUCompletionTimeline::~VulkanGPUCompletionTimeline() {
  ...
  if (!pending_submission_fences_.empty()) {
    if (vulkan_device_->functions().vkWaitForFences(vulkan_device_->device(), 1,
                                                    &pending_submission_fences_.back().second,
                                                    VK_TRUE, UINT64_MAX) == VK_ERROR_DEVICE_LOST) {
    ...
    while (!pending_submission_fences_.empty()) {
      vulkan_device_->functions().vkDestroyFence(vulkan_device_->device(),
                                                 pending_submission_fences_.back().second, nullptr);
      pending_submission_fences_.pop_back();
    }
  }

  while (!free_fences_.empty()) {
    vulkan_device_->functions().vkDestroyFence(vulkan_device_->device(), free_fences_.back(),
                                               nullptr);
...
VulkanGPUCompletionTimeline::AcquireFenceForSubmission(VkResult* const result_out_opt) {
  ...
  if (!free_fences_.empty()) {
    const VkResult fence_reset_result = vulkan_device_->functions().vkResetFences(
        vulkan_device_->device(), 1, &free_fences_.back());
    if (fence_reset_result != VK_SUCCESS) {
      REXLOG_ERROR("Failed to reset a Vulkan fence: {}",
  ...
  if (fence == VK_NULL_HANDLE) {
    ...
    const VkResult fence_create_result = vulkan_device_->functions().vkCreateFence(
        vulkan_device_->device(), &fence_create_info, nullptr, &fence);
    if (fence_create_result != VK_SUCCESS) {
      REXLOG_ERROR("Failed to create a Vulkan fence: {}",
...
VkResult VulkanGPUCompletionTimeline::AcquireFenceAndSubmit(const uint32_t queue_family_index,
  ...
  VkResult submit_result;
  {
    ...
    submit_result =
        vulkan_device_->SubmitAndUpdateLost(queue_acquisition.queue(), submit_count, submits,
...
void VulkanGPUCompletionTimeline::AwaitSubmissionImpl(const uint64_t awaited_submission) {
  ...
  if (submission_end_iterator != pending_submission_fences_.cbegin()) {
    const VkResult fence_wait_result = vulkan_device_->functions().vkWaitForFences(
        vulkan_device_->device(), 1, &std::prev(submission_end_iterator)->second, VK_TRUE,
        UINT64_MAX);
    if (fence_wait_result != VK_SUCCESS) {
      REXLOG_ERROR("Failed to wait for a Vulkan fence: {}",
    ...
    pending_submission_fences_.erase(pending_submission_fences_.cbegin(), submission_end_iterator);
  }
