...
REXCVAR_DEFINE_BOOL(sdl_high_pixel_density, false, "UI/SDL",
                    "Request high-DPI backing pixels for SDL windows")
    .lifecycle(rex::cvar::Lifecycle::kRequiresRestart);

std::unordered_map<SDL_WindowID, rex::ui::MacWindow*>& WindowMap() {
  static std::unordered_map<SDL_WindowID, rex::ui::MacWindow*> windows;
  return windows;
}

Uint32 SDLCALL AutoHideCursorTimerCallback(void* userdata, SDL_TimerID timer_id, Uint32 interval) {
  (void)interval;
  SDL_Event event = {};
...
void MacWindow::HandleSDLEvent(const SDL_Event& event) {
...
bool MacWindow::OpenImpl() {
  ...
  if (!window_) {
    REXLOG_ERROR("MacWindow: Failed to create window: {}", SDL_GetError());
    return false;
  }
  ...
  if (!metal_view_) {
    REXLOG_ERROR("MacWindow: Failed to create Metal view: {}", SDL_GetError());
    WindowMap().erase(SDL_GetWindowID(window_));
    SDL_DestroyWindow(window_);
    window_ = nullptr;
    return false;
  ...
  if (!metal_layer_) {
    REXLOG_ERROR("MacWindow: Failed to get Metal layer: {}", SDL_GetError());
    SDL_Metal_DestroyView(metal_view_);
    metal_view_ = nullptr;
    WindowMap().erase(SDL_GetWindowID(window_));
    SDL_DestroyWindow(window_);
...
void MacWindow::HandleEvent(const SDL_Event& event) {
  WindowDestructionReceiver destruction_receiver(this);
  switch (event.type) {
    case SDL_EVENT_WINDOW_EXPOSED:
      paint_event_queued_.store(false, std::memory_order_release);
...
void MacWindow::HandleSizeUpdate(WindowDestructionReceiver& destruction_receiver) {
  dpi_ = QueryDpi();
  int width = 0;
  int height = 0;
...
void MacWindow::HandleKey(const SDL_KeyboardEvent& event, bool down,
                          WindowDestructionReceiver& destruction_receiver) {
  SDL_Keymod mod = SDL_GetModState();
  KeyEvent e(this, TranslateSDLScancode(event.scancode), event.repeat ? 2 : 1, !down,
             (mod & SDL_KMOD_SHIFT) != 0, (mod & SDL_KMOD_CTRL) != 0, (mod & SDL_KMOD_ALT) != 0,
             (mod & SDL_KMOD_GUI) != 0);
  if (down) {
...
void MacWindow::HandleMouseButton(const SDL_MouseButtonEvent& event, bool down,
                                  WindowDestructionReceiver& destruction_receiver) {
...
void MacWindow::HandleMouseMotion(const SDL_MouseMotionEvent& event,
                                  WindowDestructionReceiver& destruction_receiver) {
  RevealAutoHiddenCursor();
  int32_t physical_x = 0;
  int32_t physical_y = 0;
...
void MacWindow::HandleMouseWheel(const SDL_MouseWheelEvent& event,
                                 WindowDestructionReceiver& destruction_receiver) {
  RevealAutoHiddenCursor();
  int32_t physical_x = 0;
  int32_t physical_y = 0;
...
void MacWindow::RevealAutoHiddenCursor() {
  if (GetCursorVisibility() != CursorVisibility::kAutoHidden) {
    return;
  }
...
void MacWindow::SetCursorAutoHideTimer() {
  RemoveCursorAutoHideTimer();
  if (!window_) {
    return;
  }
...
void MacWindow::RemoveCursorAutoHideTimer() {
...
void MacWindow::WindowPointToPhysical(float x, float y, int32_t& physical_x,
  ...
  if (window_ && SDL_GetWindowSize(window_, &logical_width, &logical_height) &&
      SDL_GetWindowSizeInPixels(window_, &physical_width, &physical_height) && logical_width > 0 &&
      logical_height > 0 && physical_width > 0 && physical_height > 0) {
...
uint32_t MacWindow::QueryDpi() const {
  if (!window_) {
    return kDefaultDpi;
  }
